...
Test suite mencakup: alur auth (login/2FA/password confirmation), CRUD & validasi SPK, validasi pengerjaan (terima/tolak), laporan kendala & pengerjaan, temuan kondisi, laporan bulanan, peta (endpoint data JSON), audit log, notifikasi, system error log, dan generasi PDF surat pengantar — total 126 test aktif (beberapa di-skip di lingkungan tanpa dependency tertentu).
