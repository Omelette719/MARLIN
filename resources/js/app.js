...
window.initPetaRambu = function (containerId, dataUrl, coordDisplayId, rambuDetailUrlTemplate, focusId, temuanUrlTemplate, hideTenang = false) {
    ...
    // Standard OSM tiles, for the familiar OSM look. (A CARTO tile switch
    // was tried here while chasing the PDF map-image export bug — the
    // actual cause turned out to be unrelated, a leaflet-image/marker issue
    // fixed in unduhPetaGambarPdf() below, so there was no need to move off
    // OSM after all.) crossOrigin is still needed so that function can read
    // tile pixels into a canvas.
