...
**Telegram**: `telegram_link_token` adalah token acak 32 karakter (`Str::random(32)`) yang cuma berlaku sekali dan langsung dikosongkan begitu berhasil dipakai atau digenerate ulang. Bot memvalidasi kalau satu `chat_id` Telegram sudah terhubung ke akun lain sebelum mengizinkan hubungan baru, supaya satu chat Telegram tidak bisa dipakai untuk "mencuri" notifikasi akun orang lain. Token dan chat ID tidak pernah dipakai sebagai kredensial login, cuma untuk mengarahkan ke mana pesan notifikasi dikirim.
...
**Mass assignment**: seluruh model memakai atribut PHP `#[Fillable([...])]` yang eksplisit menyebutkan kolom mana yang boleh diisi lewat `create()`/`update()`, tidak ada model yang memakai `$guarded = []` (yang berarti "semua kolom boleh diisi", rawan disalahgunakan lewat input form yang dimanipulasi).
...
- **SQL Injection**: seluruh query memakai Eloquent (parameterized query otomatis). Ada beberapa penggunaan `selectRaw`/`orderByRaw` di sistem (untuk agregasi dashboard dan pengurutan kondisi rusak-duluan), semuanya memakai string SQL tetap/statis, tidak ada input pengguna yang digabung langsung ke query SQL.
...
Aksi yang benar-benar mengubah data (`daftarkanTim`, `tambahAnggota`, `hapusAnggota`, `ajukanLaporanAkhir`) tetap punya pengecekan keanggotaan sendiri di dalam kode, jadi yang terbuka cuma **melihat**, bukan **mengubah**. Semuanya minta konfirmasi eksplisit dulu di UI sebelum benar-benar dieksekusi. `daftarkanTim` khususnya tidak bisa dibatalkan lewat sistem sama sekali (tidak ada fitur "keluar tim"); `hapusAnggota` cuma bisa menghapus baris anggota non-perwakilan, baris perwakilan (`is_perwakilan = true`) tidak pernah bisa dihapus lewat aksi ini.
