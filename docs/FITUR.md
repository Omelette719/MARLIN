...
Selain ringkasan angka tersebut, halaman ini juga menyematkan sebuah widget peta ringkas dengan filternya sendiri, terpisah dari filter yang ada di halaman Peta penuh. Filter pada widget ini mencakup jenis rambu, tingkat urgensi, kecamatan, dan kelurahan. Widget ini juga dilengkapi dengan tombol **Unduh PDF**, yang menghasilkan sebuah laporan sebaran rambu lengkap, berisi cuplikan gambar peta (mencakup tile latar belakang beserta pin-pin rambu yang ditampilkan) sesuai dengan filter yang sedang aktif pada saat tombol itu ditekan, dilengkapi dengan tabel analitik, daftar SPK aktif yang terkait, dan daftar rambu yang relevan.

Isi laporan PDF ini seluruhnya mengikuti filter yang sedang diterapkan di peta pada saat tombol diklik, bukan seluruh data sistem. Tabel **Daftar SPK Aktif Terkait** menyaring SPK di balik rambu-rambu yang lolos filter, dan hanya menampilkan yang masih berstatus Aktif (SPK yang sudah Selesai atau Dibatalkan tidak muncul di sini, karena bukan lagi pekerjaan yang sedang berjalan), dengan kolom No, Nomor Surat, Wilayah, Deadline, dan Tingkat urgensinya saat ini. Tabel **Daftar Rambu** di bawahnya berkolom No, Jenis Rambu, Lokasi, Koordinat, Kondisi (Baik, Rusak, atau N/A untuk rambu yang belum terpasang), dan SPK Terkait, kolom terakhir ini tidak ikut disaring berdasarkan status SPK, karena ia hanya menunjukkan rambu itu berasal dari SPK yang mana.
...
Field **Kelurahan** sendiri bukan teks bebas, melainkan dropdown pencarian (`x-searchable-select`) yang berisi seluruh 52 kelurahan resmi Kota Banjarmasin, dikelompokkan ke dalam 5 kecamatan (Banjarmasin Selatan, Timur, Barat, Utara, dan Tengah) menurut data "Luas Wilayah Kelurahan Menurut Kecamatan di Kota Banjarmasin" dari Sekretariat Daerah. Daftar tetap ini disimpan di `App\Support\WilayahBanjarmasin`, dipakai bersama oleh formulir SPK maupun filter Peta di bawah. Kecamatan sendiri tidak pernah disimpan sebagai kolom terpisah di mana pun, ia selalu diturunkan dari kelurahan yang dipilih setiap kali dibutuhkan.
...
Field **Deadline** wajib diisi dengan tanggal yang berada setelah hari ini, tidak boleh diisi dengan hari ini sendiri atau tanggal yang sudah lewat. Tingkat **Urgensi** dihitung secara otomatis: sisa empat hari atau kurang menuju tenggat waktu, atau ditandai sebagai **Prioritas**, akan menghasilkan urgensi Tinggi; sisa tujuh hari atau kurang menghasilkan urgensi Sedang; dan selebihnya menghasilkan urgensi Rendah.
...
Halaman ini menyajikan ringkasan berupa empat kartu angka. **Aktif** menghitung seluruh SPK berstatus aktif yang tampil pada daftar di bawahnya, terlepas dari sudah ada tim atau belum. **Progres** menghitung SPK aktif yang sudah punya tim bergabung, siapa pun timnya, bukan hanya tim petugas yang sedang login. **Mendekati Deadline** menghitung SPK aktif yang urgensinya saat ini Tinggi (baik karena sisa harinya sedikit maupun karena ditandai Prioritas), memakai perhitungan live yang sama seperti lencana urgensi pada kartunya. Ketiga kartu ini sengaja merangkum seluruh papan pekerjaan yang tampil di bawahnya, bukan hanya milik petugas yang bersangkutan, supaya angkanya tetap masuk akal bahkan sebelum petugas itu bergabung ke tim mana pun. Berbeda dengan ketiganya, **Selesai Bulan Ini** tetap bersifat pribadi, menghitung baris rambu yang sudah diselesaikan bulan berjalan lewat tim yang pernah diikuti petugas ini, karena SPK yang sudah sepenuhnya Selesai memang tidak lagi tampil pada daftar aktif di atas.

Di bawah keempat kartu itu, halaman ini juga menampilkan widget **Peta Rambu Perlu Perhatian** yang sama persis dengan yang ada pada Dashboard Admin: peta ringkas dengan filter Jenis Rambu, Tingkat, Kecamatan, dan Kelurahan, legenda warna, serta tombol **Unduh PDF** untuk mengunduh laporan sebaran rambu dari peta yang sedang ditampilkan. Filter Kecamatan dan Kelurahan memakai daftar resmi yang sama dengan dropdown Kelurahan pada formulir SPK. Bedanya, kartu pin pada widget milik petugas ini menyertakan tombol **Lapor Temuan Kondisi** langsung dari dalam kartu info pin, sama seperti pada halaman Peta Interaktif bersama, sesuatu yang sengaja tidak ditampilkan pada widget serupa di Dashboard Admin.

Selanjutnya ada dua kartu berdampingan. **SPK Perlu Perhatian** memakai logika pengurutan yang sama persis dengan widget serupa di Dashboard Admin (prioritas/urgensi tinggi lebih dulu, sisanya diurutkan dari progres paling rendah), tapi dipersempit hanya untuk SPK yang timnya sudah diikuti petugas yang sedang login, maksimal 5 baris. **Saran SPK untuk Bergabung**, di sisi lain, menyarankan SPK yang sama sekali belum ada timnya (murni papan pekerjaan yang belum diambil siapa pun), diurutkan mengutamakan yang berada di kecamatan yang sama dengan pekerjaan aktif petugas saat ini, lalu sisanya diurutkan berdasarkan jarak koordinat sungguhan (bukan sekadar tabel kedekatan kecamatan yang dihardcode) dari titik tengah seluruh rambu yang sedang ia kerjakan, maksimal 3 baris. Kalau petugas belum mengikuti satu pun SPK aktif sama sekali, sehingga tidak ada titik acuan untuk dibandingkan, saran ini otomatis jatuh kembali ke urutan berdasarkan deadline yang paling dekat. Pada kedua kartu ini, nomor surat SPK bisa langsung diklik menuju halaman Detail SPK yang bersangkutan.
...
| Biru tua | Selesai dan kondisinya baik, pin ini juga satu-satunya yang tidak berkedip |
| Kuning | Urgensi Sedang |
| Abu-abu | Urgensi Rendah, atau belum ada tugas aktif sama sekali untuk dijadikan acuan urgensi |
...
Halaman ini secara default menampilkan **seluruh** rambu tanpa terkecuali, termasuk yang sudah selesai dikerjakan dan kondisinya baik (pin biru tua yang tidak berkedip), berbeda dari widget peta ringkas di Dashboard yang secara default menyembunyikan pin semacam itu. Untuk mempersempit tampilan, halaman ini dilengkapi filter Jenis Rambu, Tingkat, Kecamatan, dan Kelurahan, sama seperti pada widget Dashboard, dengan satu perbedaan: pilihan Tingkat di sini turut menyertakan "Selesai / Kondisi Baik" sebagai opsi yang bisa dipilih secara eksplisit, karena halaman ini tidak punya bias "perlu perhatian" seperti widget Dashboard. Tombol **Unduh PDF** di halaman ini menghasilkan laporan yang sama persis strukturnya dengan yang dihasilkan dari Dashboard, mengikuti filter yang sedang aktif pada peta ini.
